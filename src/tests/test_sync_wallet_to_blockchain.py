...
def mock_client():
    ...
    mock_client.block_is_orphan = lambda block: True if block.hash == ORPHANED_BLOCK_3_HASH else False
    mock_client.get_utxos_by_address = lambda address: address.utxos
    return mock_client
